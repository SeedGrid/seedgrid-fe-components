...
test("module.json expõe o manifesto do módulo billing-user", async () => {
  const manifest = JSON.parse(await readFile(manifestPath, "utf8"));

  assert.equal(manifest.id, "billing-user");
  assert.deepEqual(manifest.requires, ["security"]);
  assert.equal(manifest.configKey, "billing-user");
  assert.equal(manifest.variantGroup, undefined);
  assert.equal(manifest.provider, undefined);
  assert.deepEqual(manifest.navigation, {
    import: "useBillingUserAppShellSections",
    from: "@/modules/billing-user",
  });
  assert.deepEqual(manifest.messages, {
    import: "billingUserMessages",
