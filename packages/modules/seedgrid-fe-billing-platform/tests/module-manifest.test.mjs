...
test("module.json expõe o manifesto do módulo billing-platform", async () => {
  const manifest = JSON.parse(await readFile(manifestPath, "utf8"));

  assert.equal(manifest.id, "billing-platform");
  assert.deepEqual(manifest.requires, ["security"]);
  assert.equal(manifest.configKey, "billing-platform");
});
