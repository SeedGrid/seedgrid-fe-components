...
import type { CreatePlanTermRequest } from "@seedgrid/fe-billing-platform";
...
export async function GET(_request: NextRequest, context: Ctx) {
  ...
  return toRouteResponse(async () => {
    ...
    return plans.listTerms(publicId);
  });
}

export async function POST(request: NextRequest, context: Ctx) {
  const { publicId } = await context.params;
  const body = (await request.json()) as CreatePlanTermRequest;

  return toRouteResponse(async () => {
    const plans = await getBillingPlatformServer();
    return plans.createTerm(publicId, body);
  });
}
